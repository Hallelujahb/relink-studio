...
echo "=== Relink Studio setup ==="

# Work relative to this script's own location, wherever the zip was
# unpacked to -- not a hardcoded ~/Downloads.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# ------------------------------------------------------------------------ #
# Backend (relink-api): Python venv, deps, optional linktransformer,
# then start the Flask dev server in the background on :5000.
# ------------------------------------------------------------------------ #
cd "$SCRIPT_DIR"
if [ ! -d "relink-api" ]; then
  echo "ERROR: relink-api/ not found next to this script."
  echo "Make sure you unzipped the whole archive (this script + relink-api/ +"
  echo "relink_studio.jsx + test-dataset/ should all be siblings), then re-run it."
  exit 1
fi
cd relink-api

if ! command -v python3 &> /dev/null; then
  echo "python3 not found. Installing (this will ask for your password)..."
  sudo apt update && sudo apt install -y python3 python3-venv python3-pip
fi

if [ ! -d ".venv" ]; then
  echo "Creating a Python virtual environment for the backend..."
  python3 -m venv .venv
fi
...
if [ "$INSTALL_LT" == "1" ]; then
  ...
  "$VENV_PIP" install -q linktransformer
  echo "Installed. Method C is now enabled -- its model downloads from Hugging Face Hub"
  echo "the first time you actually run a project with Method C turned on."
else
  echo "Skipping linktransformer. Method C will return a clear 'requires linktransformer'"
  echo "error in the review UI until you re-run this script with --with-linktransformer"
  echo "(or RELINK_INSTALL_LINKTRANSFORMER=1 ./setup_relink_studio.sh)."
fi

echo "Starting backend API on http://localhost:5000 ..."
RELINK_CORS_ORIGINS="http://localhost:5173" nohup "$VENV_PY" run.py > "$SCRIPT_DIR/relink-api/backend.log" 2>&1 &
BACKEND_PID=$!
echo "Backend PID: $BACKEND_PID -- logs at $SCRIPT_DIR/relink-api/backend.log"
trap 'echo "Stopping backend (PID $BACKEND_PID)..."; kill $BACKEND_PID 2>/dev/null' EXIT

echo -n "Waiting for backend to respond on :5000"
BACKEND_UP=0
for i in $(seq 1 10); do
  if curl -s -o /dev/null http://localhost:5000/api/projects/healthcheck; then
    BACKEND_UP=1
    break
  fi
  echo -n "."
...
if [ "$BACKEND_UP" == "1" ]; then
  ...
  echo "WARNING: backend still isn't responding after 10s. Last 30 lines of backend.log:"
  echo "----------------------------------------------------------------------"
  tail -n 30 "$SCRIPT_DIR/relink-api/backend.log"
  echo "----------------------------------------------------------------------"
  echo "Continuing anyway -- fix the error above and restart the backend manually if needed:"
  echo "  cd relink-api && .venv/bin/python run.py"
fi
...
if ! command -v node &> /dev/null; then
  echo "Node.js not found. Installing (this will ask for your password)..."
...
cd "$SCRIPT_DIR"
if [ -d "relink-studio" ]; then
  echo "relink-studio folder already exists, using it."
  cd relink-studio
else
  echo "Creating new vite project..."
  npm create vite@latest relink-studio -- --template react --no-interactive
  cd relink-studio
fi
...
echo "Writing public/favicon.svg..."
mkdir -p public
rm -f public/vite.svg
cat > public/favicon.svg << 'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 22" fill="none">
  <circle cx="7" cy="7" r="4.5" stroke="#3B82F6" stroke-width="2" />
  <circle cx="15" cy="15" r="4.5" stroke="#3B82F6" stroke-width="2" />
  <line x1="9.8" y1="9.8" x2="12.2" y2="12.2" stroke="#3B82F6" stroke-width="2" stroke-linecap="round" />
</svg>
EOF
...
echo "=== Setup complete. Backend running at http://localhost:5000. Starting frontend dev server on http://localhost:5173 ... ==="
npm run dev
